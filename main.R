...
# convert to factor (to create categorical var for prediction)
...
predict_survival <- function(model, new_sample) {
	...
	cat("Probability of survival: ", survival_prediction[, 2], "(Threshold: 0.6)\n")
...
printAccuracy <- function() {
	...
	accuracy <- mean(predictions == test_data$Survived) # mean used for proportion of correct predictions
...
# function for various data visualizations
...
while (TRUE) {
	...
	if (choice == 1) {
		...
		cat("Survival Prediction:", predict_survival(model, newdata), "\n")
	} else if (choice == 2) {
		printAccuracy()
	} else if (choice == 3) {
